...
myKeys c =
  ...
  , ("M-S-d", addName "Run prompt"             $ sequence_ [spawn (mySoundPlayer ++ dmenuSound), spawn "~/.local/bin/dm-run"])
  , ("M-S-<Return>", addName "Launch file browser"      $ spawn (myFile))
  ...
  [ ("M-g", addName "Shrink window"               $ sendMessage Shrink)
  , ("M-h", addName "Expand window"               $ sendMessage Expand)
